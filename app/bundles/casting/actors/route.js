  ...
  ActorAttributeFinder, {
    ...
    afterModel(model) {
      ...
      if (Em.isEmpty(role)) {
        ...
        this.controllerFor('casting/actors').set('matchedGridConfiguration', this.matchedGridConfiguration(model));
        this.controllerFor('casting/actors').set('unMatchedGridConfiguration', this.unMatchedgridConfiguration(model));
        this.controllerFor('casting/actors').set('appointedgridConfiguration', this.appointedgridConfiguration(model));
        this.controllerFor('casting/actors').set('invitedGridConfiguration', this.invitedGridConfiguration(model));
      }
    },
    ...
    getCastingStep(casting) {
      ...
      if (!published) {
        if (castingStep == 2) {
          this.controllerFor('casting/actors').set('infoMessage', this.i18n.t('casting.add.step2'));
        } else if (castingStep == 3) {
          this.controllerFor('casting/actors').set('infoMessage', this.i18n.t('casting.add.step3'));
        } else if (castingStep == 4) {
          this.controllerFor('casting/actors').set('infoMessage', this.i18n.t('casting.add.step4'));
        } else if (castingStep == 5) {
          this.controllerFor('casting/actors').set('infoMessage', this.i18n.t('casting.add.step5'));
        }
      }
    },
    ...
    invitedGridConfiguration(model) {
      return {
        defaultView: true,
        modelName: 'actor',
        headerName: 'Invited actors',
        gridHeight: 65,
        gridHeader: false,
        gridAsList: false,
        additionals: {
        ...
        columns: [{
          ...
          valuePath: 'self',
          width: '250px',
          cellComponent: 'actor-invited-casting-content'
        }],
      };
    },
    ...
    actions: {
      setUp(tab) {
        this.cookie.removeCookie('tab_actor');
        this.cookie.setCookie('tab_actor', tab, {
          expires: 7,
          path: '/'
        });
      },
